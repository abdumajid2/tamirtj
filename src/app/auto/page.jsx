// app/auto/page.jsx  (или где у тебя этот роут)
// "Автосервис" = categoryId: 6
...
import {
  LuCar, LuBadgeCheck, LuStar, LuPhone, LuSearch, LuFilter, LuCircleCheck
} from 'react-icons/lu';

const BRAND = { green: '#00B140', greenLight: '#E8F8EE', dark: '#111827' };

// Подкатегории для Автосервиса в твоём server.cjs (id: 61..66)
const AUTO_SUBS = {
  61: 'ТО/масло',
  62: 'Тормоза',
  63: 'Подвеска',
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {subs.slice(0, 3).map((id) => (
              <Chip key={id}>{AUTO_SUBS[id] ?? `Услуга #${id}`}</Chip>
            ))}
...
export default function AutoPage() {
  ...
  // Бэкенд отдаёт всех авто-мастеров при categoryId:6
  const { data: masters = [], isLoading } = useGetMastersQuery({ categoryId: 6 });
    ...
    <section className="max-w-6xl mx-auto p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          >
            <LuCar className="text-2xl" style={{ color: BRAND.green }} />
          </span>
          <div>
            <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900">Авто-мастера</h1>
            <div className="text-sm text-gray-500">Надёжные специалисты по авто в вашем городе</div>
          </div>
        </div>
      ...
      {/* Быстрый фильтр по подкатегориям */}
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {Object.entries(AUTO_SUBS).map(([id, name]) => {
